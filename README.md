...
**Живой деплой:** **[uniguide-aggregator.netlify.app](https://uniguide-aggregator.netlify.app)**
...
для этого не подходит: он доступен только на чтение. `src/lib/db.ts` решает это без догадок о
хостинге: при первом обращении к базе в рантайме он реальной проверкой (`fs.accessSync(path,
W_OK)`) смотрит, доступен ли файл, на который указывает `DATABASE_URL`, для записи. Если да —
ничего не меняется (локальная разработка, самохостящийся деплой с настоящей персистентной БД).
Если файл существует, но доступен только на чтение (бандл serverless-функции), он копирует
засеянную БД в `/tmp/dev.db` один раз на холодный старт и открывает Prisma-клиент через опцию
`datasourceUrl` уже поверх этой копии — код запросов не меняется.
> Две более ранние версии этой проверки гадали о хостинге вместо проверки факта: сначала —
> `process.env.NETLIFY`/`VERCEL` (эта переменная оказалась недоступна в рантайме функции Netlify,
> только на этапе сборки — первое сообщение в чат падало с `SQLITE_READONLY` на живом деплое),
> затем — `NODE_ENV === "production"` (сработало на Netlify, но код-ревью верно указало, что это
> сломало бы самохостящийся деплой с настоящей персистентной БД). Подробности — в
> [`WORKFLOW.md`](WORKFLOW.md), раздел «Рефлексия».
