...
- Не вводить вторую систему стилизации (CSS-модули, styled-components и т. п.) параллельно с
  Tailwind.
- Формат ответов ассистента при работе над этой ролью: правки сразу в виде diff/патча к
  конкретному файлу компонента, без прозы, без обсуждения альтернатив, если не задан явный вопрос.
...
Для этой роли не использовались — работа над компонентами велась напрямую поверх общего слоя
запросов в `src/lib/universities.ts`, без делегирования отдельным сабагентам.
